...
Software development at Buncombe County follows a release strategy that provides
...
(na) kicks off unit and functional tests which pass before changes are
...
Buncombe County practices continuous delivery of code into production
...
Buncombe County’s multiple lower environments (dev, test) provide an ecosystem of
...
All Buncombe County `git` repositories have a company standard configuration from a
...
().  Continuous integration (na) kicks off unit and functional
...
Buncombe County practices continuous delivery of code into production through multiple
...
Buncombe County development/engineering team uses  for
...
All Buncombe County software must be developed to include the following general
...
reported and tracked following Buncombe County Vulnerability Management process
...
Buncombe County Security Team in collaboration with development team performs full
...
Documentation on the Buncombe County [Engineering Wiki]()
may include additional security specifications as well as the security design
and implementation details of the Buncombe County Platform and its
...
Buncombe County security and development team implemented a process to
...
functionality is embedded into the `securityScan()` na pipeline step.
...
-   The Lint and GitLeaks Platform - static and dynamic code analysis. Together with
    ...
    Buncombe County na pipeline library via the `securityScan()` step
...
Buncombe County requires all outsourced software development to follow the
