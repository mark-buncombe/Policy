...
Buncombe County defines the following four classifications of data:
...
of retention, are defined according to the Buncombe County Data
Classifications.
...
Buncombe County customer, or as required by regulations, whichever is
...
Buncombe County stores data in a secure production account in AWS, using a
...
Buncombe County performs automatic backup of all customer and system data to protect
...
Customers can also utilize the Buncombe County Application Programming Interface (API)
to extract and store their data elsewhere. Standard API usage fees will apply.

#### Source code

Buncombe County stores its source in git repositories hosted by .

Source code repositories are backed up to Buncombe County’s AWS S3 infrastructure
...
Buncombe County SharePoint team site. Examples of business files include, but are not
limited to:
...
Despite not being a requirement within HIPAA, Buncombe County understands and
appreciates the importance of health data retention. Acting as a
subcontractor/service provider, and at times a business associate, Buncombe County is
not directly responsible for health and medical records retention as set forth
by each state.

Buncombe County has created and implemented the following procedures to
make it easier for Buncombe County Customers to support data retention
laws.
...
Buncombe County is NOT a covered entity.  Should we become a covered entity in the
