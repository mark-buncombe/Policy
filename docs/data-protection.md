...
Data is classified and handled according to the Buncombe County Data Handling
...
identified by its location/repository. For example, source codes in Buncombe County’s
...
approved encryption standards. More details are specified in Buncombe County’s Data
...
Buncombe County hosts on Amazon Web Services in the US-East (Ohio) region by default.
Data is replicated across multiple regions for redundancy and disaster recovery.

All Buncombe County employees, systems, and resources adhere to the following standards
...
3. Ensure Buncombe County Customer Production Data is segmented and only accessible to
   Customer authorized to access data.
4. All Production Data at rest is stored on encrypted volumes using encryption
   keys managed by Buncombe County. Encryption at rest is ensured through the use of
...
Buncombe County employee access to production is guarded by an approval process and by
...
Buncombe County uses AWS CloudWatch/CloudTrail to monitor the entire cloud service
...
Buncombe County uses a security agent to monitor production systems. The
...
built into Amazon Machine Images (AMIs) for use in Buncombe County AWS
environments.
...
1. All external data transmission is encrypted end-to-end using encryption keys
   managed by Buncombe County. This includes, but is not limited to, cloud
...
5. For all Buncombe County APIs, enforcement of authentication, authorization, and
